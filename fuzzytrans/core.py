"""
Core fuzzy logic functions for FuzzyTrans library.
...
def triangular_fuzzy_number(a: float, b: float, c: float, x: float) -> float:
    """
    Calculate triangular fuzzy number.
    ...
    Returns:
        Membership function value at point x
...
def gaussian_fuzzy_set(x: float, sigma: float, c: float) -> float:
    """
    Calculate Gaussian fuzzy set.
    ...
    Returns:
        Gaussian fuzzy function value at point x

    """
    return np.exp(-((x - c) ** 2) / (2 * sigma**2))


def bell_function(x: float, a: float, b: float, c: float) -> float:
    """
    Calculate generalized bell function.
    ...
    Returns:
        Generalized bell function value at point x
...
def f_A_downB(
    ...
    """
    values = [
        bell_function(x, a, b, c) * F_A
        for c, F_A in zip(centers, F_A_down, strict=False)
    ]
    return max(values)
...
def f_A_downG(
    ...
    """
    values = [
        gaussian_fuzzy_set(x, sigma, c) * F_A
        for c, F_A in zip(centers, F_A_down, strict=False)
    ]
    return max(values)
